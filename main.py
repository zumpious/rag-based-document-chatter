...
def process_query(
    query: str, chat_history: List[Dict[str, str]], k_value: int, fetch_k: int
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "raw_docs": raw_docs,
            "similarity_scores": [
                doc.metadata.get("score", "N/A") if hasattr(doc, "metadata") else "N/A"
                for doc in raw_docs
            ],
        }
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        fetch_k = st.slider(
            "Number of candidates to consider (fetch_k)",
            ...
            with st.spinner("Researching..."):
                ...
                with st.expander("🔍 RAG Debug Information", expanded=True):
                    ...
                    if response["has_context"]:
                        st.markdown(
                            f"**Number of chunks retrieved:** {len(response['source_documents'])}"
                        )

                        for idx, (doc, score) in enumerate(
                            zip(response["raw_docs"], response["similarity_scores"]), 1
                        ):
